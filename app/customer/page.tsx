...
export default function CustomerPage() {
  ...
  if (!isMounted || !isCartLoaded) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-50 via-stone-50 to-teal-700/10">
        <div className="text-center">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-full bg-gradient-to-tr from-teal-700 to-cyan-700 mb-4 shadow-2xl">
  ...
  const EmptyState = () => (
    <div className="text-center py-20">
      <div className="mx-auto inline-flex items-center justify-center w-20 h-20 rounded-xl bg-indigo-50 shadow-sm mb-6">
        <ShoppingCart className="w-9 h-9 text-indigo-700" strokeWidth={1.6} />
      </div>
    ...
    <div className="group flex items-center gap-4 p-4 bg-white rounded-xl border border-stone-100 hover:shadow-md transition-shadow duration-150">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-4 text-xs text-stone-500 mt-1">
          ...
          <div className="flex items-center gap-1">
            <span className="text-stone-600">Qty:</span>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-stone-50 to-teal-700/10 py-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-xl bg-gradient-to-tr from-teal-700 to-cyan-700 flex items-center justify-center shadow-2xl">
                  <ShoppingCart className="w-7 h-7 text-white" strokeWidth={1.6} />
                </div>
                <div>
              ...
              <div className="hidden md:flex items-center gap-3">
                <div className="text-sm text-stone-600">{cart.length} items</div>
                <Button onClick={() => { setCart([]); localStorage.removeItem('customer_cart'); toast({ title: 'Cleared', description: 'Cart cleared.' }) }} variant="ghost" size="sm" className="text-stone-500 hover:text-rose-600">Clear</Button>
              </div>
            </div>
              ...
              <CardHeader>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription className="text-sm text-stone-500">Tap your phone on a tag to add items instantly.</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent>
                <div className="flex gap-4 items-center">
                  <Button onClick={handleNfcRead} disabled={!isNfcSupported || isNfcReading} className="flex-1 h-12 rounded-lg bg-gradient-to-r from-teal-700 to-cyan-700 hover:from-teal-800 hover:to-cyan-800 text-white shadow-md">
                    {isNfcReading ? (
                      <>
                  ...
                  <div className="w-48 text-sm text-stone-500">
                    {isNfcSupported ? (
            ...
            {/* Cart list */}
            <Card className="rounded-2xl shadow-md border border-stone-100 overflow-hidden">
              <CardHeader className="border-b">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg bg-indigo-50 flex items-center justify-center">
                      <ShoppingCart className="w-5 h-5 text-indigo-700" strokeWidth={1.6} />
                    </div>
                    <div>
                      <CardTitle className="text-lg font-semibold">Items</CardTitle>
                      <CardDescription className="text-sm text-stone-500">Review your items before checkout</CardDescription>
                    </div>
                  </div>

                  <div>
                    {cart.length > 0 && (
                      <Button onClick={handleCheckout} className="bg-gradient-to-r from-teal-700 to-cyan-700 text-white shadow-sm">Checkout</Button>
                    )}
                  </div>
                </div>
              </CardHeader>
              ...
              <CardContent>
                {cart.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="mt-4 bg-white rounded-xl p-4 border border-stone-100 shadow-sm">
                      <div className="flex justify-between items-center mb-2">
                        <div className="text-sm text-stone-600">Subtotal</div>
                        <div className="font-semibold text-stone-800">₹{totals.subtotal.toFixed(2)}</div>
                      </div>

                      {discount > 0 && (
                        <div className="flex justify-between items-center bg-teal-50 p-3 rounded-lg border border-teal-100 mb-3">
                          <div className="flex items-center gap-2 text-sm text-teal-700">
                            <Gift className="w-4 h-4" />
                            <span>Discount</span>
                          </div>
                          <div className="font-semibold text-teal-800">-₹{discount.toFixed(2)}</div>
                        </div>
                      )}

                      <div className="flex justify-between items-center pt-3 border-t">
                        <div className="text-lg font-semibold">Total</div>
                        <div className="text-2xl font-bold text-teal-800">₹{totals.total.toFixed(2)}</div>
                      </div>

                      {/* Checkout button below total */}
                      <div className="mt-4">
                        <Button onClick={handleCheckout} className="w-full h-12 bg-gradient-to-r from-teal-700 to-cyan-700 text-white rounded-lg shadow-md">Proceed to Checkout</Button>
                      </div>
                    </div>
                  </div>
                )}
            ...
            <div className="sticky top-24">
              <div className="rounded-2xl p-6 bg-white shadow-sm border border-stone-100">
                ...
                <div className="space-y-3">
                  ...
                  {/* Checkout button below totals in sidebar as well */}
                  <div className="mt-4">
                    <Button onClick={handleCheckout} className="w-full h-12 bg-gradient-to-r from-teal-700 to-cyan-700 text-white rounded-lg shadow-md">Proceed to Checkout</Button>
                  </div>
      ...
      <Dialog open={isCheckoutOpen} onOpenChange={setIsCheckoutOpen}>
        <DialogContent className="relative max-w-2xl w-[95vw] sm:w-full bg-white rounded-2xl shadow-2xl p-6 overflow-auto">
          {/* Dismiss (cross) */}
          <button aria-label="Close checkout" onClick={() => setIsCheckoutOpen(false)} className="absolute right-4 top-4 w-9 h-9 rounded-full bg-stone-50 hover:bg-stone-100 flex items-center justify-center">
          ...
          <div className="mt-6 space-y-5">
            {/* Phone */}
            <div>
              <Label htmlFor="phone" className="text-sm font-medium text-stone-700 mb-2 block">Phone number</Label>
              <div className="flex gap-3">
                <div className="relative flex-1">
                  <Phone className="absolute left-3 top-1/2 transform -translate-y-1/2 text-stone-400 w-4 h-4" strokeWidth={1.6} />
                  <Input id="phone" type="tel" placeholder="Enter your phone number" value={phone} onChange={(e) => setPhone(e.target.value)} className="pl-10 h-12 text-black placeholder:text-stone-400" />
                </div>
                <Button onClick={sendOTP} disabled={otpSent} className="h-12 px-4 bg-gradient-to-r from-teal-700 to-cyan-700 text-white">
                  {otpSent ? (
                    <>
            ...
            {otpSent && (
              <div>
                ...
                <div className="flex gap-3 items-center">
                  <Input id="otp" value={otp} onChange={(e) => setOtp(e.target.value)} maxLength={6} className="h-12 text-center font-semibold tracking-widest text-lg text-black placeholder:text-stone-400" disabled={otpVerified} />
                  {otpVerified ? (
                    <Button disabled className="h-12 bg-teal-50 text-teal-700">Verified</Button>
                  ) : (
                    <Button onClick={verifyOTP} disabled={isVerifying} className="h-12 px-4 bg-gradient-to-r from-teal-700 to-cyan-700 text-white">
                      {isVerifying ? "Verifying..." : "Verify"}
                    </Button>
                  )}
                </div>
                {otpVerified && <div className="mt-2 text-sm text-emerald-600">Phone number verified ✓</div>}
              </div>
            )}
            ...
            <div>
              ...
              <div className="flex gap-3">
                <Input id="coupon" value={couponCode} onChange={(e) => setCouponCode(e.target.value)} className="h-12 text-black placeholder:text-stone-400" />
                <Button onClick={applyCoupon} className="h-12 px-4 bg-gradient-to-r from-indigo-700 to-indigo-800 text-white"><Tag className="w-4 h-4 mr-2" />Apply</Button>
              </div>
            </div>
                ...
                <label className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition ${paymentMethod === 'pay_at_desk'? 'border-teal-300 bg-teal-50/40': 'border-stone-100 bg-white hover:border-teal-100'}`}>
                  ...
                  <div className="flex items-center gap-3 flex-1">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-stone-500">Pay when you pick up</div>
                    </div>
                  </div>
                </label>
              </RadioGroup>
            ...
            <div className="pt-4 border-t">
              <div className="flex items-center justify-between mb-3">
                <div className="text-sm text-stone-600">Total payable</div>
                <div className="text-xl font-bold text-teal-800">₹{totals.total.toFixed(2)}</div>
              </div>

              <Button onClick={processPayment} disabled={!otpVerified || !paymentMethod} className="w-full h-12 bg-gradient-to-r from-teal-700 to-cyan-700 text-white rounded-lg shadow-md">
                {!otpVerified ? (
                  <>
      ...
      {paymentConfirmed && confirmedOrderId && (
        <div className="fixed left-1/2 -translate-x-1/2 bottom-6 z-50">
          <div className="bg-white rounded-xl shadow-xl p-4 border border-stone-100 flex items-center gap-4">
            ...
            <div>
              <div className="font-semibold text-stone-800">Payment Confirmed</div>
              <div className="text-sm text-stone-500">Order <span className="font-medium">{confirmedOrderId}</span> placed successfully.</div>
            </div>
            <div className="ml-4 flex items-center gap-2">
              {confirmedBillUrl && (
                <Button onClick={() => window.open(confirmedBillUrl!, '_blank')} size="sm" className="bg-teal-700 text-white"><Download className="w-4 h-4 mr-2" />Bill</Button>
              )}
